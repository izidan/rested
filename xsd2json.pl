 ...
 *
 * Hold the default `_Value` to flag value attribute names in
 *   `xs:simpleContent`.
 */
value_attribute('@').
...
node(IName,Namespace,attribute,Attribute_ID,_Attribute_Children,_Parent_ID)
  ==>
    xsd_namespace(Namespace)
  |
    node_attribute(IName,Attribute_ID,type,string,default).
...
/**
 * `xs:sequence` within a `xs:extension`
 */
transform(IName),
    node(IName,NS1,extension,Extension_ID,_Extension_Children,_Extension_Parent_ID),
    node(IName,NS2,sequence,Sequence_ID,_Sequence_Children,Extension_ID),
    json(IName,Sequence_ID,Sequence_JSON)
  ==>
    xsd_namespaces([NS1,NS2])
  |
    json(IName,Extension_ID,Sequence_JSON).
...
/**
 * `xs:sequence` which has a `xs:choice` child.
 */
transform(IName),
    node(IName,NS1,sequence,Sequence_ID,_Sequence_Children,_Sequence_Parent_ID),
    node(IName,NS2,choice,Choice_ID,_Choice_Children,Sequence_ID),
    json(IName,Choice_ID,Choice_JSON)
  ==>
    xsd_namespaces([NS1,NS2])
  |
    json(IName,Sequence_ID,Choice_JSON).


/**
 * `xs:sequence` within a `xs:choice`
 */
transform(IName),
    node(IName,NS1,choice,Choice_ID,_Choice_Children,_Choice_Parent_ID),
    node(IName,NS2,sequence,Sequence_ID,_Sequence_Children,Choice_ID),
    json(IName,Sequence_ID,Sequence_JSON)
  ==>
    xsd_namespaces([NS1,NS2])
  |
...
/**
 * `xs:group` which has a `xs:sequence` child.
 */
transform(IName),
    node(IName,NS1,group,Group_ID,_Group_Children,_Group_Parent_ID),
    node(IName,NS2,sequence,Sequence_ID,_Sequence_Children,Group_ID),
    json(IName,Sequence_ID,Sequence_JSON)
  ==>
    xsd_namespaces([NS1,NS2])
  |
    json(IName,Group_ID,Sequence_JSON).


/**
 * `xs:group` within `{xs:complexType,xs:sequence}` which has a group with `@ref`
 *   attribute being set.
 */
transform(IName),
    node(IName,NS1,ParentEl,ParentEl_ID,_ParentEl_Children,_ParentEl_Parent_ID),
    node(IName,NS2,group,Group_ID,_Group_Children,ParentEl_ID),
    node_attribute(IName,Group_ID,ref,Ref,_)
  ==>
    (
  ...
  |
    ...
    ;
      ParentEl == sequence,
      json(IName,_ParentEl_Parent_ID,json(JSON))
    ).
...
/**
 * `xs:extension` which has `xs:attributeGroup` child.
 */
transform(IName),
    node(IName,NS1,simpleContent,SimpleContent_ID,_SimpleContent_Children,_SimpleContent_Parent_ID),
    node(IName,NS2,extension,Extension_ID,_Extension_Children,SimpleContent_ID),
    node(IName,NS3,attributeGroup,AttributeGroup_ID,_AttributeGroup_Children,Extension_ID),
    node_attribute(IName,AttributeGroup_ID,ref,Ref,_),
    node_attribute(IName,Extension_ID,base,Base,_)
  ==>
    xsd_namespaces([NS1,NS2,NS3]),
    value_attribute(_Value),
    reference_type(Ref,json(Ref_JSON)),
    reference_type(Base,json(Base_JSON))
  |
    json(IName,_SimpleContent_Parent_ID, json([
      type= object,
      allOf=[ json(Ref_JSON) ],
      properties=json([
        _Value= json(Base_JSON)
      ]),
      required= [_Value]
    ])).
...
transform(IName),
  ...
  ==>
    xsd_namespaces([NS1,NS2])
  |
    json(IName,ComplexType_ID,Choice_JSON).
...
transform(IName),
  ...
  ==>
    ...
    lookup(properties,Extension_JSON_List_Wo_Base,json(Properties),Extension_JSON_List_Wo_Base_Facets)
  |
...
transform(IName),
  ...
  |
    ...
    (
        Use == required,
        JSON2 = [required=[Attribute_Name2]|JSON1]
      ;
        Use \= required,
        JSON2 = JSON1
    ),
...
transform(IName),
  ...
  ==>
    ...
    xsd_namespaces([NS1,NS2]),
    mark_attribute(Mark),
    string_concat(Mark,_Attribute_Name,Attribute_Name2)
  |
    string_concat('#/definitions/',Ref,Definition_Ref),
    Attribute_JSON = [
      '$ref'=Definition_Ref
...
transform(IName),
  ...
  |
    ...
    mark_attribute(Mark),
    string_concat(Mark,Attribute_Name,Definition_Name),
...
/**
 * `xs:simpleType` which has a `xs:union` child.
 */
transform(IName),
    node(IName,NS1,simpleType,SimpleType_ID,_SimpleType_Children,_SimpleType_Parent_ID),
    node(IName,NS2,union,Union_ID,_Union_Children,SimpleType_ID),
    node_attribute(IName,Union_ID,memberTypes,MemberTypes,_)
  ==>
    xsd_namespaces([NS1,NS2]),
    string_concat(MemberTypes," ",MemberTypes0),
    split_string(MemberTypes0," "," ",MemberTypes1),
    convlist(reference_type,MemberTypes1,MemberTypes2)
  |
    json(IName,SimpleType_ID,json([
      allOf=MemberTypes2
    ])).
...
/**
 * Create `schema_definition/3` constraint for each
 *   `attributeGroup` node which has the `@name` attribute
 *   set.
 */
transform(IName),
    node(IName,NS1,attributeGroup,AttributeGroup_ID,_AttributeGroup_Children,_AttributeGroup_Parent_ID),
    json(IName,AttributeGroup_ID,AttributeGroup_JSON),
    node_attribute(IName,AttributeGroup_ID,name,Definition_Name,_)
  ==>
    xsd_namespaces([NS1])
  |
    schema_definition(IName,Definition_Name,AttributeGroup_ID,AttributeGroup_JSON).

/**
 * Create `schema_definition/3` constraint for each
 *   `group` node which has the `@name` attribute
 *   set.
 */
transform(IName),
    node(IName,NS1,group,Group_ID,_Group_Children,_Group_Parent_ID),
    json(IName,Group_ID,Group_JSON),
    node_attribute(IName,Group_ID,name,Definition_Name,_)
  ==>
    xsd_namespaces([NS1])
  |
    schema_definition(IName,Definition_Name,Group_ID,Group_JSON).
