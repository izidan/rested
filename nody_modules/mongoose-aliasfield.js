    ...
    return function toAliasedFieldsObject(){
        ...
        for(var p in schema.paths){
            ...
            if(this.get(property.path) !== undefined){
                ...
                }else if (property.options.alias !== false){
                    document[property.path] = this.get(property.path);
                }
            }
        }
        for(var p in schema.virtuals){
            var property = schema.virtuals[p];
            if(property.options.ref && 'string' == typeof property.options.ref && property.options.ref !== ''){
                document[property.path] = property.getters[property.getters.length - 1].call(this, (this.$$populatedVirtuals||{})[property.path]);
            }
        }
        return document;
    };
